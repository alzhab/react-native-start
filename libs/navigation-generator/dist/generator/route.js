"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const path_1 = __importDefault(require("path"));
const indexes_1 = __importDefault(require("./indexes"));
const utils_1 = require("../utils");
function generateRoute(route, genPath) {
    return __awaiter(this, void 0, void 0, function* () {
        const modulePath = path_1.default.join(genPath, route.name);
        const haveChilds = !!route.routes;
        yield utils_1.generateFolder(modulePath);
        yield indexes_1.default(modulePath, route.routes, route.type);
        if (haveChilds) {
            for (const childRoute of route.routes) {
                yield generateRoute(childRoute, modulePath);
            }
        }
        return;
    });
}
exports.default = generateRoute;
